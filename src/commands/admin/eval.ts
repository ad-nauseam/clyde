...
import { Team, CommandInteraction, MessageEmbed } from 'discord.js'
...
async function _eval(interaction: CommandInteraction) {
    ...
    interaction.reply(evaled.length > 2000 
...
const command: SlashCommand = {
	...
	async execute(interaction) {
		...
		if(owner instanceof Team) {
            ...
            if(owner?.id == interaction.user.id) await _eval.call(this, interaction);
        }
	}
};
